...
const ADDR_CONTROL: u16 = 0;
const ADDR_MASK: u16 = 1;
const ADDR_STATUS: u16 = 2;
const ADDR_OAM_ADDRESS: u16 = 3;
const ADDR_OAM_DATA: u16 = 4;
const ADDR_SCROLL: u16 = 5;
const ADDR_PPU_ADDRESS: u16 = 6;
const ADDR_PPU_DATA: u16 = 7;
...
impl Color {
    pub const BLACK: Self = Self::from_rgb(u8::MIN, u8::MIN, u8::MIN);
    pub const WHITE: Self = Self::from_rgb(u8::MAX, u8::MAX, u8::MAX);
...
impl PixelBuffer {
    ...
    #[inline]
    pub const fn get_pixels(&self) -> &[Color] {
        &self.pixels
    }
...
impl ObjectAttributes {
    ...
    const fn attr(&self) -> SpriteAttributes {
        SpriteAttributes::from_bits_truncate(self.attribs[2])
    }

    #[inline]
    const fn palette(&self) -> u8 {
        (self.attribs[2] & 0x03) + 0x04
    }

    #[inline]
    fn dec_x(&mut self) {
        self.attribs[3] = self.attribs[3].wrapping_sub(1);
    }
}
...
impl Ppu {
    ...
    fn get_palette_color(&self, bus: &mut PpuBus<'_>, palette: u16, pixel: u8) -> Color {
        // A pixel with value of 0 always mirrors to the first color in the palette (background)
        const BASE_ADDR: u16 = 0x3F00;
    ...
    fn load_background_data(&mut self, bus: &mut PpuBus<'_>) {
        match (self.cycle - 1) % 8 {
            0 => {
                self.load_shifters();
                self.bg_next_id = self.read_bus(bus, 0x2000 | (self.vram_addr.value & 0x0FFF));
            }
            2 => {
                self.bg_next_attr = self.read_bus(
                    bus,
                    0x23C0
            ...
            4 => {
                ...
                self.bg_next_lsb = self.read_bus(bus, addr);
            }
            6 => {
                let bg_table = self.control.contains(PpuControl::PATTERN_BACKGROUND);
                let offset = select(bg_table, 1 << 12, 0);
                let addr = offset + ((self.bg_next_id as u16) << 4) + self.vram_addr.fine_y + 8;
                self.bg_next_msb = self.read_bus(bus, addr);
            }
            7 => self.inc_x(),
            _ => {}
        }
    ...
    fn load_foreground_data(&mut self, bus: &mut PpuBus<'_>) {
        if (self.cycle == MAX_CYCLE) && (self.scanline >= 0) {
            ...
            for i in 0..self.sprite_count {
                ...
                let mut pattern_lo = self.read_bus(bus, addr_lo);
                let mut pattern_hi = self.read_bus(bus, addr_hi);
            ...
            ADDR_PPU_ADDRESS => {
                if self.ppu_addr_latch {
                    ...
                    self.tram_addr.value =
                        (self.tram_addr.value & 0x00FF) | (((data & 0x3F) as u16) << 8);
                    self.tram_addr.update_subfields();
                }
                self.ppu_addr_latch = !self.ppu_addr_latch;
            }
